...
public class Client {
	...
	public static void startDriverSelector(Client client) {
		...
		Platform.runLater(() -> {
			...
			try {
				URI uri = client.getClass().getResource("/Servers.txt").toURI();
			...
			listView.setOnMouseClicked(e -> {
				if (e.getButton().equals(MouseButton.PRIMARY) && e.getClickCount() == 2) {
					...
					client.startBank(stage, configStr.split("\\s"));
				}
			});
	...
	private Menu createTestMenu() {
		...
		).map(Client::loadTest).filter(t -> t != null).collect(Collectors.toList());
